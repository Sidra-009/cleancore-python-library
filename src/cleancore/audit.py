...
def audit_trail(
    ...
    - Input snapshot is taken BEFORE calling the function so that in-place
      mutations are correctly detected as schema/value drift.
    """
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            if _is_dataframe(raw_input):
                input_snapshot = raw_input.copy()   # snapshot before mutation
                output_data = func(raw_input, *rest_args, **kwargs)
                input_data  = input_snapshot
            else:
                input_data = materialise(raw_input)
                # Deep-copy so in-place mutations inside func don't corrupt snapshot
                ...
                input_data = input_snapshot   # use clean snapshot for diff
